...
export default function StatCard({
  ...
  return (
    <SurfaceCard variant={variant} hover="lift" className={cn("stat-card", className)}>
      <div className="stat-card__body flex flex-col gap-3 p-5">
        ...
        <span
          ref={valueRef}
          dir="ltr"
          className="stat-card__value type-metric text-[var(--color-foreground)]"
        >
          {displayValue}
        </span>
        {item.note ? (
          <span className="stat-card__note type-metric-secondary text-[var(--color-foreground-soft)]">
            {item.note}
          </span>
        ) : null}
      </div>
